...
bool MeshPositionFilter::get_index(const Tracker& tktr, 
                                    std::array<int, 3>& indices )override final {
    ...
    if ( (index_x >= 0 && index_x < Nx_) 
    && ( index_y >= 0 && index_y < Ny_) 
    && (index_z >= 0 && index_z < Nz)_ ){
        indices[0] = index_x;
        indices[1] = index_y;
        indices[2] = index_z;
