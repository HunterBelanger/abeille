...
class CylinderPositionFilter : public PositionFilter {
 public:
  ...
  CylinderPositionFilter(Position origin_, double radius_, double dx, double dy,
                         double dz_, size_t nx_ = 1, size_t ny_ = 1,
                         size_t nz_ = 1, Orientation z_ = Orientation::Z)
      : origin(origin_),
        new_origin_(),
        Nx_(nx_),
        Ny_(ny_),
    ...
    if (radius <= 0.0) fatal_error("Incorrect value of radius is provided.");

    if ((pitch_x == 0.0) && (Nx_ != 1)) {
      warning(
          "x-direction pitch is zero, Only one will be considered in "
          "x-direction.");
      Nx_ = static_cast<size_t>(1);
      inv_pitch_x = 0.0;
    } else {
      if (pitch_x < (2 * radius)) fatal_error("Pitch is less than the radius");
      inv_pitch_x = 1.0 / pitch_x;
    }

    if ((pitch_y == 0.0) && (Ny_ != 1)) {
      warning(
          "y-direction pitch is zero, Only one will be considered in "
          "y-direction.");
      Ny_ = static_cast<size_t>(1);
      inv_pitch_y = 0.0;
    } else {
      if (pitch_y < (2 * radius)) fatal_error("Pitch is less than the radius ");
  ...
  std::size_t Nx() const override {
    if (axial_dir == Orientation::X) return Nz_;

    return Nx_;
  }

  std::size_t Ny() const override {
    if (axial_dir == Orientation::Y) return Nz_;

    return Ny_;
  }

  std::size_t Nz() const override {
    if (axial_dir == Orientation::X) return Nx_;

    if (axial_dir == Orientation::Y) return Ny_;
  ...
  Position new_origin() const {
    return new_origin_;
  }  // to get the new_origin for zernike; z() will zmin
 ...
 private:
  ...
  double radius, pitch_x, pitch_y, dz, inv_pitch_x, inv_pitch_y,
      inv_dz;  // dz is not required to store, neither length_z, only inv_dz is
               // sufficient

  // To map the indexes to either converting into class co-ordinate or into
  // original
  void map_indexes(std::array<int, 3>& indexes) const {
    if (axial_dir == Orientation::Z) return;
...
bool CylinderPositionFilter::get_indices(const Tracker& tktr,
  ...
  if (nx >= 0 && nx < static_cast<int>(Nx_) && ny >= 0 &&
    ...
    if (sqrt((new_origin_x - r.x()) * (new_origin_x - r.x()) +
      ...
      new_origin_ =
          Position(new_origin_x, new_origin_y, (origin.z() + nz * dz));
