...
class ZernikePolynomials{
    public:
        ...
        double evaluate_zernike_at_order(const double x, const double theta, const std::size_t order)const ;

        std::vector<double> evaluate_zernikes(const double r, const double theta) const;
    ...
    private:
        ...
        std::pair<int, int> get_n_and_l(const std::size_t& order )const {
            ...
            if ( m == j + 1){
                ...
                std::cout<<"the n and m for zernike is not found.\n";
            }
            return {0, 0};
        }
};
