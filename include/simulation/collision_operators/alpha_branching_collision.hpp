 ...
 *
 * */
#ifndef ALPHA_BRANCHING_COLLISION_H
#define ALPHA_BRANCHING_COLLISION_H

#include <materials/material.hpp>
#include <simulation/collision_operators/alpha_fission_operator.hpp>
...
class AlphaBranchingCollision {
 public:
  AlphaBranchingCollision() = default;

  void write_output_info(const std::string& base) const {
    if (mpi::rank != 0) return;

    auto& h5 = Output::instance().h5();
    h5.createAttribute(base + "collision-operator", "alpha-branching");
  }
  ...
  void collision(Particle& p, MaterialHelper& mat,
    ...
    // Get alpha parameters. We assume that the optional must be filled here
    const double eta = mat.alpha_params()->eta;
    const double alpha = mat.alpha_params()->alpha;
    ...
    fission_operator.fission(p, xs, *nuclide, alpha);

    // Make a copy of the particle when doing alpha transport, modifying
    ...
    // Implicit capture
    p.set_weight(p.wgt() * (1. - (xs.absorption + xs.alpha) / xs.total));
    p.set_weight2(p.wgt2() * (1. - (xs.absorption + xs.alpha) / xs.total));
 ...
 private:
  AlphaFissionOperator fission_operator;
  ...
  void alpha_copy(Particle& p, const MicroXSs& xs, double eta) const {
    if (xs.alpha / xs.total + p.rng() >= 1.) {
      const double yield{(eta + 1) / eta};
      p.make_secondary(p.u(), p.E(), p.wgt() * yield);
    }
  }
};
