...
// BoxPositionFilter::get_indices_tracklength(Position r, const Direction& u_,
// double d_flight)
...
void BoxPositionFilter::check_on_boundary(const Tracker tktr,
                                          std::array<int, 3> on) {
...
std::shared_ptr<BT> make_box_position_filter(const YAML::Node &node){
    if (!node["low"])
        fatal_error("For box position-filter \"low\" co-ordinates is not
provided."); if (!node["high"]) fatal_error("For box position-filter \"high\"
co-ordinates is not provided.");
