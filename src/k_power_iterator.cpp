...
void KPowerIterator::initialize() {
...
void KPowerIterator::write_output_info() const {
...
void KPowerIterator::set_cancelator(std::shared_ptr<Cancelator> cncl) {
  cancelator = cncl;
...
void KPowerIterator::set_entropy(const Entropy& entropy) {
...
void KPowerIterator::add_source(std::shared_ptr<Source> src) {
  if (src)
...
void KPowerIterator::load_source_from_file() {
  // Load source file
...
void KPowerIterator::sample_source_from_sources() {
...
void KPowerIterator::print_header() {
...
void KPowerIterator::generation_output() {
  std::stringstream output;
...
void KPowerIterator::run() {  // Would put alpha update here
  Output& out = Output::instance();
...
void KPowerIterator::comb_particles(std::vector<BankedParticle>& next_gen) {
  double Wpos_node = 0.;
  double Wneg_node = 0.;
...
void KPowerIterator::write_entropy_families_etc_to_results() const {
  if (mpi::rank != 0) return;
...
void KPowerIterator::normalize_weights(std::vector<BankedParticle>& next_gen) {
  double W = 0.;
  double W_neg = 0.;
  double W_pos = 0.;
...
void KPowerIterator::compute_pre_cancellation_entropy(
...
void KPowerIterator::compute_post_cancellation_entropy(
    std::vector<BankedParticle>& next_gen) {
  if (t_pre_entropy && cancelator) {
    for (size_t i = 0; i < next_gen.size(); i++) {
...
void KPowerIterator::compute_pair_dist_sqrd(
...
void KPowerIterator::zero_entropy() {
  if (p_pre_entropy) {
    p_pre_entropy->zero();
  }
...
void KPowerIterator::premature_kill() {
...
bool KPowerIterator::out_of_time(std::size_t gen) {
...
void KPowerIterator::check_time(std::size_t gen) {
...
void KPowerIterator::perform_regional_cancellation(
    std::vector<BankedParticle>& next_gen) {
...
std::shared_ptr<KPowerIterator> make_k_power_iterator(const YAML::Node& sim) {
  ...
  // Create simulation
  std::shared_ptr<KPowerIterator> simptr =
      std::make_shared<KPowerIterator>(mover);
