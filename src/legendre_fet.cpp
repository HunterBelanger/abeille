...
LegendreFET::LegendreFET(std::shared_ptr<CartesianFilter> position_filter_, 
    ...
    cartesian_filter_ (position_filter_), energy_in_(energy_in), 
    axes(axes_), FET_order(FET_order_)
    {
    
    //for legendre, check the size of axis vector should be between than 3.
    if ( axes.size() <= 1 && axes.size() >=3 )
        fatal_error("The no. of given axis for legendre-FET is not between 1 to 3 for tally " + name_ + ".");
    ...
    if ( cartesian_filter_ ){
        tally_dimensions_ = cartesian_filter_->get_dimension();
        check_filter = false;
    }
    else{
        fatal_error("For the tally, " + name_ + ", the positional filter is required to do the legendre functional expansion tally.");
    }
...
void LegendreFET::score_collision(const Particle& p, const Tracker& tktr, MaterialHelper& mat ){
    ...
    std::vector<size_t> position_index_;
    std::size_t index_E;
    bool check_filter = true;

    // if cartesian_filter_ exist than get the indexes
    if ( cartesian_filter_ ){
    ...
    bool axes_size_more_than_1 = (axes.size() > 1);
    if ( axes_size_more_than_1 ){
        indexes_.push_back(0);
    }

    // Check weather, given- FET order is more than 1 or not.
    const size_t FET_index = indexes_.size();
    bool order_more_than_0 = (FET_order >  0); 
    if ( order_more_than_0 ){
        indexes_.push_back(0);
    }
    ...
    for ( auto& c : axes){  // Loop over the different axis and indexing is done using the it_axis
        ...
        for (size_t i = 0; i<FET_order+1; i++){    // loop over differnt FET order
            beta_n = collision_score * legendre(i, scaled_loc); // score for i-th order's basis function

            // if FET order is more than zero, than assigned that location otherwise, no assignment  
            if ( order_more_than_0 ){ 
...
std::shared_ptr<ITally> make_legendre_fet(const YAML::Node& node){
    ...
    if ( estimator_name == "collision" ){
        itally_legendre_fet_tally_ = std::make_shared<LegendreFET>(cartesian_filter_, energy_filter_,
                                                     fet_axis, FET_order_, LegendreFET::Quantity::Flux, 
