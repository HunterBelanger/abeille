...
LegendrePolynomials::LegendrePolynomials(std::size_t order)
  ...
  if (order_ != 0){
    if (order_ % 2 == 0){
      const std::size_t length = static_cast<std::size_t>((order_-1)/2 + 1);
      coeff_length = length * (length + 1) + static_cast<std::size_t>(order_/2 + 1);
    } else {
      const std::size_t length = static_cast<std::size_t>(order_/2 + 1);
