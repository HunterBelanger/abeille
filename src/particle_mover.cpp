...
using TransporterObjct = std::variant<SurfaceTracker, DeltaTracker, ImplicitLeakageDeltaTracker, CarterTracker>;
...
std::shared_ptr<IParticleMover> make_concrete_mover(const YAML::Node& sim, const CollOp& cop) {
    TransporterObjct top = make_transporter(sim);

    if (std::holds_alternative<SurfaceTracker>(top)) {
        return std::make_shared<ParticleMover<SurfaceTracker, CollOp>>(std::get<SurfaceTracker>(top), cop);
    } else if (std::holds_alternative<DeltaTracker>(top)) {
        return std::make_shared<ParticleMover<DeltaTracker, CollOp>>(std::get<DeltaTracker>(top), cop);
    } else if (std::holds_alternative<ImplicitLeakageDeltaTracker>(top)) {
        return std::make_shared<ParticleMover<ImplicitLeakageDeltaTracker, CollOp>>(std::get<ImplicitLeakageDeltaTracker>(top), cop);
    } else if (std::holds_alternative<CarterTracker>(top)) {
        return std::make_shared<ParticleMover<CarterTracker, CollOp>>(std::get<CarterTracker>(top), cop);
    }
