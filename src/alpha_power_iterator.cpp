...
void AlphaPowerIterator::initialize() {
...
void AlphaPowerIterator::write_output_info() const {
...
void AlphaPowerIterator::set_cancelator(std::shared_ptr<Cancelator> cncl) {
  cancelator = cncl;
...
void AlphaPowerIterator::set_entropy(const Entropy& entropy) {
...
void AlphaPowerIterator::add_source(std::shared_ptr<Source> src) {
  if (src)
...
void AlphaPowerIterator::load_source_from_file() {
  // Load source file
...
void AlphaPowerIterator::sample_source_from_sources() {
...
void AlphaPowerIterator::print_header() {
...
void AlphaPowerIterator::generation_output() {
  std::stringstream output;

  double kstep = Tallies::instance().k_alpha();
  ...
  if (gen <= nignored + 1) {
    ...
    double kstep_avg = Tallies::instance().k_alpha_avg();
    double kstep_err = Tallies::instance().k_alpha_err();
    output << "   " << alpha_avg << " +/- " << alpha_err << "   " << kstep_avg
           << " +/- " << kstep_err;
  }
  output << "   ";
...
void AlphaPowerIterator::run() {
  Output& out = Output::instance();
  out.write(" Running Alpha Eigenvalue Problem...\n");
  ...
  for (gen = 1; gen <= ngenerations; gen++) {
    ...
    std::vector<BankedParticle> next_gen = particle_mover->transport(
        bank, std::nullopt, nullptr, nullptr, alpha_params);
    ...
    normalize_weights_and_calc_kstep(next_gen);

    // Update alpha
    if (alpha_params.alpha > 0) {
      alpha_params.alpha *= alpha_params.keff;
    } else if (alpha_params.alpha < 0) {
      alpha_params.alpha /= alpha_params.keff;
    }
    Tallies::instance().set_alpha(alpha_params.alpha);
...
void AlphaPowerIterator::normalize_weights_and_calc_kstep(
  ...
  // Calculate k_step
  alpha_params.keff = W / static_cast<double>(nparticles);
...
void AlphaPowerIterator::compute_pre_cancellation_entropy(
    std::vector<BankedParticle>& next_gen) {
...
void AlphaPowerIterator::compute_post_cancellation_entropy(
    std::vector<BankedParticle>& next_gen) {
  if (t_pre_entropy && cancelator) {
    for (size_t i = 0; i < next_gen.size(); i++) {
...
void AlphaPowerIterator::compute_pair_dist_sqrd(
...
void AlphaPowerIterator::zero_entropy() {
  if (p_pre_entropy) {
    p_pre_entropy->zero();
  }
...
void AlphaPowerIterator::premature_kill() {
...
bool AlphaPowerIterator::out_of_time(std::size_t gen) {
...
void AlphaPowerIterator::check_time(std::size_t gen) {
...
std::shared_ptr<AlphaPowerIterator> make_alpha_power_iterator(
  ...
  std::shared_ptr<IParticleMover> mover =
      make_particle_mover(sim, settings::SimMode::ALPHA);
  ...
  AlphaParameters alpha_params;
  alpha_params.eta = 1.;
  alpha_params.keff = 1.;
  if (sim["eta"] && sim["eta"].IsScalar()) {
    alpha_params.eta = sim["eta"].as<double>();
    if (alpha_params.eta <= 0.) {
      fatal_error("Eta must be greater than 0.");
    }
  } else if (sim["eta"]) {
    fatal_error("Invalid eta entry in a-eigenvalue simulation.");
  }

  if (!sim["alpha"]) {
    fatal_error("Alpha value required for a-eigenvalue simulation.");
  }
  if (sim["alpha"].IsScalar() == false) {
    fatal_error("Alpha must be a scalar value.");
  }
  alpha_params.alpha = sim["alpha"].as<double>();

  if (alpha_params.alpha == 0.) {
    fatal_error("Alpha cannot be 0.");
  }

  // Create simulation
  std::shared_ptr<AlphaPowerIterator> simptr =
      std::make_shared<AlphaPowerIterator>(mover, alpha_params);
