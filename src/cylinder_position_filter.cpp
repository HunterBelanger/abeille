...
CylinderPositionFilter::CylinderPositionFilter(Position origin, double radius, double dx, double dy, double dz, 
                    std::size_t nx, std::size_t ny, std::size_t nz, Orientation z_ )
      : origin_(origin), r_low_(),
        Nx_(nx),
        Ny_(ny),
        Nz_(nz),
        length_axis_(z_),
    ...
    if (radius_ <= 0.) fatal_error("A non-zero value of radius must be provided.");
    inv_radius_ = 1.0 / radius_;

    // Check for a non-zero pitch_x_ and pitch_y_ should be more than cylinder diameter
    if ((pitch_x_ == 0.) && (Nx_ != 1)) {
      warning(
          str_pitch_x + "-direction pitch is zero, Only one will be considered in "
          "that-direction.");
      ...
      warning(
          str_pitch_x + "-direction pitch is non-zero, but the bin is one in that direction,so, Only one will be considered in "
          "that-direction.");
    } else {
      if ( pitch_x_!= 0. &&  pitch_x_ < (2. * radius_)) fatal_error(str_pitch_x + "-pitch is less than the radius");
      inv_pitch_x_ = 1.0 / pitch_x_;
    }

    if ((pitch_y_ == 0.) && (Ny_ != 1)) {
      warning(
          str_pitch_y + "-direction pitch is zero, Only one will be considered in "
          "the direction.");
      ...
      inv_pitch_y_ = 0.;
        ...
        warning(
          str_pitch_y + "-direction pitch is non-zero, but the bin is one in that direction,so, Only one will be considered in "
          "that-direction.");
    } else {
      if ( pitch_y_!=0. && pitch_y_ < (2. * radius_)) fatal_error(str_pitch_y + "pitch is less than the radius ");
...
Position CylinderPositionFilter::get_centre(const StaticVector3&  indices) const {
    // the indicies is not according to the class, so map it.
    StaticVector3 index = indices;
    map_indexes(index);
...
std::pair<double, double> CylinderPositionFilter::get_scaled_radius_and_angle(const StaticVector3& indices, const Position& r) const{
    ...
    const double new_origin_x = origin_.x() + pitch_x_ * static_cast<double>(index[0]);
    const double new_origin_y = origin_.y() + pitch_y_ * static_cast<double>(index[1]);
    // scaled-radius
...
std::shared_ptr<CylinderPositionFilter> make_cylinder_position_filter(const YAML::Node& node){
    // check and get the origin
    if ( !node["origin"]){
        fatal_error("the origin must be provided for the cylinder position filter.");
    ...
    if (!node["axial-axis"]){
        fatal_error("axial-direction axis must be given for the cylinder position filter.");
    } else if (!node["axial-axis"].IsScalar()){
        fatal_error("Invalid axial-axis is given for the cylinder position filter.");
    }
    ...
    if ( !node[str_pitch_x]){
        if ( nx != 1){
            warning(str_pitch_x + " is not given for the cylinder position filter, so there will be one cylinder is x-direction.");
        }
        nx = 1;
    } else {
        if( !node[str_pitch_x].IsScalar() ){
            fatal_error("Invalid " + str_pitch_x + " is given for the cylinder position filter.");
        }
        pitch_x = node[str_pitch_x].as<double>();
        if ( pitch_x == 0. && nx != 1){
            warning("for the cylinder position filter, the " + str_pitch_x + " is 0.0, so, there will be 1 cylinder in x-direction.");
            nx = 1;
        }
    }
    ...
    if ( !node[str_pitch_y]){
        if ( nx != 1){
            warning(str_pitch_y + " is not given for the cylinder position filter, so there will be one cylinder is x-direction.");
        }
        nx = 1;
    } else {
        if( !node[str_pitch_y].IsScalar() ){
            fatal_error("Invalid " + str_pitch_y + " is given for the cylinder position filter.");
        }
        pitch_y = node[str_pitch_y].as<double>();
        if ( pitch_y == 0. && nx != 1){
            warning("for the cylinder position filter, the " + str_pitch_y + " is 0.0, so, there will be 1 cylinder in x-direction.");
            nx = 1;
        }
    }
    ...
    if ( axial_axis == "x" || axial_axis == "X" ){ 
        ...
        dy = pitch_y;
        dz = pitch_x;
        cylinder_filter = std::make_shared<CylinderPositionFilter>(origin, radius, 
                                                    dx, dy, dz, nx, ny, nz,
