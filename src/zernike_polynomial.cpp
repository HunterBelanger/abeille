...
ZernikePolynomials::ZernikePolynomials(std::size_t order)
    :    
    order_(order), 
    max_n_(), Zr_coefficients_(), m_(), n_(), Zr_types_(){
// Evaluate the polynomial coefficients up to given order
// Zr_coefficients_ will be having the coeff. stating from lowest power to higher
// for each order, it will be useful this way as evaluating the power can easily be done.
...
for ( std::size_t it = 0; it <= order_; it++){ 
    ...
    for ( std::size_t i = 0; i <= max_k; i++ ){
        ...
        const double denominator = static_cast<double>(factorial(k) * factorial(avg_n_m - k) * factorial ( mid_n_m - k));
...
double ZernikePolynomials::evaluate_zernike_at_order(const double x, const double theta, const std::size_t order)const {
...
std::vector<double> ZernikePolynomials::evaluate_zernikes(const double r, const double theta) const {
